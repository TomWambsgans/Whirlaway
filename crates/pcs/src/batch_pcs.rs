...
pub trait BatchPCS<FA: Field, FB: Field, EF: ExtensionField<FA> + ExtensionField<FB>> {
    ...
    fn batch_verify(
        ...
        parsed_commitment: &<Self::PcsA as PCS<FA, EF>>::ParsedCommitment,
        statements_a: &[Evaluation<EF>],
    ...
    [PF<EF>; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
{
    ...
    fn batch_open(
        ...
        let config = WhirConfig::new(self.0.clone(), polynomial_a.num_variables());
    ...
    fn batch_verify(
        &self,
        verifier_state: &mut FSVerifier<EF, impl FSChallenger<EF>>,
        parsed_commitment: &<Self::PcsA as PCS<F, EF>>::ParsedCommitment,
        statements_a: &[Evaluation<EF>],
        parsed_commitment_b: &<Self::PcsB as PCS<EF, EF>>::ParsedCommitment,
        statements_b: &[Evaluation<EF>],
    ) -> Result<(), ProofError> {
        let config = WhirConfig::new(self.0.clone(), parsed_commitment.num_variables);
        let mut whir_statements_a = Statement::new(parsed_commitment.num_variables);
        for statement in statements_a {
            whir_statements_a.add_constraint(statement.point.clone(), statement.value);
        }
