...
use crate::{
    CoefficientListCuda, MAX_LOG_N_BLOCKS, cuda_alloc, cuda_get_at_index, cuda_info, memcpy_htod,
};
...
/// (+ reverse vars)
/// Async
pub fn cuda_monomial_to_lagrange_basis<F: Field>(coeffs: &CudaSlice<F>) -> CudaSlice<F> {
...
// Async
pub fn cuda_whir_fold<F: Field>(
    coeffs: &CoefficientListCuda<F>,
    folding_randomness: &[F],
) -> CoefficientListCuda<F> {
    let folding_factor = folding_randomness.len() as u32;
    let n_vars = coeffs.n_vars() as u32;
    let cuda = cuda_info();
    let n_threads_per_blocks = MULTILINEAR_LOG_N_THREADS_PER_BLOCK.min(coeffs.n_coefs() as u32);
    let n_blocks = (((coeffs.n_coefs() / 2) as u32 + n_threads_per_blocks - 1)
    ...
    let mut res = cuda_alloc::<F>(coeffs.n_coefs() / (1 << folding_factor) as usize);
    let f = cuda.get_function("multilinear", "whir_fold");
    let mut launch_args = cuda.stream.launch_builder(&f);
    launch_args.arg(coeffs.coeffs());
    launch_args.arg(&n_vars);
