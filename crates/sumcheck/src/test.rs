use super::*;
use algebra::pols::{Multilinear, eval_sumcheck_computation, univariate_selectors};
use arithmetic_circuit::{CircuitComputation, SumcheckComputation, TransparentPolynomial};
...
fn test_sumcheck() {
    ...
    let exprs = (0..n_exprs)
        ...
        .collect::<Vec<_>>();
    let eq_factor = (0..n_vars).map(|_| rng.random()).collect::<Vec<_>>();
    ...
    let multilinears = (0..n_multilinears)
        .map(|_| Multilinear::<EF>::random(rng, n_vars))
        .collect::<Vec<_>>();

    let batching_scalar: EF = rng.random();
    let batching_scalars = powers(batching_scalar, n_exprs);
    let eq_mle = Multilinear::eq_mle(&eq_factor);
    let sum = Multilinear::compute_over_hypercube(
        &multilinears,
    ...
    println!("sumcheck: {} ms", time.elapsed().as_millis());
...
fn test_univariate_skip() {
    ...
    let multilinears = (0..n_multilinears)
        .map(|_| Multilinear::<F>::random(rng, n_vars))
        .collect::<Vec<_>>();
    let batching_scalar: EF = rng.random();
    let batching_scalars = powers(batching_scalar, n_exprs);
    ...
    let selector_evals = selectors
        ...
        .collect::<Vec<_>>();
    let folded_multilinears_host = multilinears
        .iter()
